...
- **Versioning — [.github/workflows/version.yml](.github/workflows/version.yml)** — on every merge (push) to `main`, creates a standard SemVer tag and GitHub Release in `v<major>.<minor>.<build>` format (for example, `v1.0.3`). The workflow reads `package.json` `version` as the requested `major.minor.build` release line, auto-increments the build number from existing tags in that major/minor line, and preserves `x.y.0` when a new major/minor line has no existing `v<x>.<y>.*` tags. The next version is computed by [scripts/next-version.mjs](scripts/next-version.mjs), the single source of truth shared with the CI `changelog` guard and the `/ship` skill.
