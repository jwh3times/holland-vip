...
- **Post-deploy smoke — [.github/workflows/smoke.yml](.github/workflows/smoke.yml)** — daily cron (+ manual `workflow_dispatch`) curls the live `https://holland.vip`, asserting HTTP 200, expected content, and the security headers. This is the only check that exercises the _deployed_ site rather than the pre-deploy build. Because holland.vip is behind Cloudflare — which 403s datacenter/runner IPs like GitHub's Azure-hosted runners — each curl sends a secret `X-Smoke-Check` header matched by a Cloudflare WAF **Skip** rule; the header value is the `SMOKE_TOKEN` repo secret. If `SMOKE_TOKEN` is unset the run warns and will 403.
