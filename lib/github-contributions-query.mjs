...
/**
 * Upper bound on any single contribution count and on the calendar total.
 *
 * A structural `typeof x === "number"` check admits negatives, fractions,
 * `NaN`, and `Infinity`, none of which can be a real contribution count and all
 * of which reach the heatmap's sizing and colour maths. The bound is far above
 * any plausible real value; it exists to reject nonsense, not to cap a user.
 */
const MAX_CONTRIBUTIONS = 1_000_000;

/** A finite, non-negative integer no greater than `max`. */
function isCountLike(value, max) {
  return Number.isInteger(value) && value >= 0 && value <= max;
}

/**
 * An ISO `YYYY-MM-DD` date naming a real calendar day.
 *
 * The round-trip rejects well-shaped impossibilities like `2026-02-31`, which a
 * regex alone would accept and `Date` would silently roll into March.
 */
...
export function parseCalendar(data) {
  ...
  if (!isCountLike(data.totalContributions, MAX_CONTRIBUTIONS)) return null;
    ...
    (week) =>
      ...
      week.every(
        (day) =>
          ...
          day !== null &&
          isIsoDate(day.date) &&
          isCountLike(day.count, MAX_CONTRIBUTIONS) &&
