...
3. **Branch — is this a multi-session build?**
   ...
   Either way, **`/implement`** builds each issue by driving **`/tdd`** internally — one red-green slice at a time — then closes out by running **`/code-review`**, addressing its findings, and committing. Reach for **`/tdd`** on its own when you just want to build a concrete behaviour test-first without a full spec, and **`/code-review`** on its own whenever you want to review a branch or PR against a fixed point.

4. **`/ship`** — when the branch's implementation commits are complete, refresh the repo docs, write the versioned changelog entry CI requires, regenerate and verify agent artifacts, run the fast gates, push, and open or update the PR. A commit is the end of implementation, not the end of this repo's delivery flow.
...
- **`/wizard`** — for the steps only a **human** can take: provisioning infrastructure, setting up credentials or CI secrets, clicking through an unfamiliar third-party dashboard, running a one-off migration or cutover. It generates an interactive bash script that opens each URL, captures each value, and routes it to the right place — configuration into `.env`, credentials into GitHub secrets or 1Password with only a reference persisted — so the procedure stops being something you re-explain to an agent every time. Model-invoked, so the agent reaches for it the moment it hits a wall only you can pass. If the agent could just do it itself, it should; this is for where a human is genuinely in the loop.
