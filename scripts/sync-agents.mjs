#!/usr/bin/env node
// Regenerate the generated agent artifacts from their canonical sources:
//   .agents/skills/** → .claude/skills/**   (skill mirror for Claude Code)
//   .claude/agents/*.md → .codex/agents/*.toml (subagents for Codex)
// Usage:
...
if (changed.length === 0 && stale.length === 0) {
  console.log("Generated agent artifacts are in sync.");
  process.exit(0);
}

if (check) {
  console.error(
    "::error::Generated agent artifacts are out of sync. Run `npm run sync:agents` and commit the result."
  );
